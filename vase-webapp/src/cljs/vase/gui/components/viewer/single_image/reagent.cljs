(ns vase.gui.components.viewer.single-image.reagent
  (:require  [goog.events :as gevents]
             [reagent.core :as r]
             [vase.gui.components.viewer.util
              :refer [set-max-size! forward-keydown-p forward-wheel-p]]
             [vase.gui.components.progress.reagent
              :as reagent-progress]))
    ...
    (r/create-class
     {:component-did-mount
      ...
      (fn [{:keys [image-url thumbnails progress viewer-select-list
                   on-diff]}]
        [:div {:class "vase-component-singleimageviewer"}

         [:div {:class "vase-component-singleimageviewer-left"
                :style {:position "absolute" :float "left"}}
          [:div {:class "vase-component-singleimageviewer-image"}
           [:div {:on-wheel on-wheel :style {:display "inline-block"}}
            [:img {:src image-url :ref #(reset! image-elem %)}]]]]

         [:div {:class "vase-component-singleimageviewer-right"}
